...
class FilterCache : public Cache {
    ...
    public:
        ...
        inline uint64_t load(Address vAddr, uint64_t curCycle, uint32_t signature = 0) {
            ...
            if (vLineAddr == filterArray[idx].rdAddr) {
                ...
                return replace(vLineAddr, idx, true, curCycle, signature);
            }
        }

        inline uint64_t store(Address vAddr, uint64_t curCycle, uint32_t signature = 0) {
            Address vLineAddr = vAddr >> lineBits;
            uint32_t idx = vLineAddr & setMask;
            uint64_t availCycle = filterArray[idx].availCycle; //read before, careful with ordering to avoid timing races
            if (vLineAddr == filterArray[idx].wrAddr) {
                ...
                return replace(vLineAddr, idx, false, curCycle, signature);
            }
        }

        uint64_t replace(Address vLineAddr, uint32_t idx, bool isLoad, uint64_t curCycle, uint32_t signature) {
            Address pLineAddr = procMask | vLineAddr;
            MESIState dummyState = MESIState::I;
            futex_lock(&filterLock);
            MemReq req = {pLineAddr, isLoad? GETS : GETX, 0, &dummyState, curCycle, &filterLock, dummyState, srcId, reqFlags, signature};
